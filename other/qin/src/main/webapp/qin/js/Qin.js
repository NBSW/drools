...
Q._init = function () {
    ...
    document.write('<link href="' + Q.config.webContext + '/' + Q.config.qinPath + '/plugin/jquery.poshytip/tip-green/tip-green.css" rel="stylesheet" type="text/css"\/>');
    document.write('<link href="' + Q.config.webContext + '/' + Q.config.qinPath + '/plugin/jquery.poshytip/tip-skyblue/tip-skyblue.css" rel="stylesheet" type="text/css"\/>');
    document.write('<link href="' + Q.config.webContext + '/' + Q.config.qinPath + '/plugin/jquery.poshytip/tip-violet/tip-violet.css" rel="stylesheet" type="text/css"\/>');
...
Q._ajax = function () {
                        ...
                        if (null != Q.devData[urlKey]) {
                            for (var methodKey in Q.devData[urlKey]) {
                                if (type.toUpperCase() === methodKey.toUpperCase()) {
                                    var target = Q.devData[urlKey][methodKey];
                                    if ($.isFunction(target)) {
                                        target = target(param);
                                    }
                                    if (undefined != customSuccessFun && null != customSuccessFun) {
                                        customSuccessFun({code: 200, msg: '', body: target});
                                        if (!isAsync) {
                                            return target;
                                        }
                                    } else {
                                        undefined != successFun && null != successFun && successFun(target);
                                        if (!isAsync) {
                                            return target;
                                        }
                                    }
    ...
    return {
        ajax: function (param) {
            return ajax(param.method, param.url, param.data, param.success, param.error, param.modal, 'JSON', 'JSON', param.async, param.before, param.complete, param.custom);
        },
...
Q._view = function () {
    ...
    function renderHelp($scope) {
        if (!$scope) {
            $scope = $('body');
        }
        $('*[title]:not([position])', $scope).poshytip({
    ...
    function renderTip($target, content, clazz, isShow) {
        $target.poshytip({
            className: 'tip-' + clazz,
            alignTo: 'target',
            alignX: 'center',
    ...
    return {
        ...
        renderHelp: function ($scope) {
            renderHelp($scope);
        },
        error: function ($target, content) {
            renderTip($target, content, 'violet', true);
        },
        tooltip: function ($target, content) {
            renderTip($target, content, 'skyblue', true);
        }
    };
};
...
Q._form = function () {
    ...
    function fill(data, $form, prefixName, isReFetchEle) {
        var $ele;
        var eleName;
        var value;
        var dataKey;
        ...
        $form.find('*[name]').each(function () {
            ...
            if (isReFetchEle) {
                //when data cascade scene the cascaded element will be rebuild so we must refetch it .
                $ele= $form.find('*[name="'+ eleName+'"]');
            }
            if (-1 != _.indexOf(filledEleName, eleName)) {
                return;
            }
            filledEleName.push(eleName);
            value = null;
            ...
            dataKey = eleName;
            if (-1 !== dataKey.indexOf('[]')) {
                dataKey = dataKey.substring(0, dataKey.indexOf('[]'));
            }
            dataKey = prefixName && dataKey.indexOf(prefixName) != -1 ? dataKey.substring(prefixName.length) : dataKey;
            try {
                value = eval('data["' + dataKey + '"]');
            } catch (e) {
            }
            if (undefined !== value && null !== value) {
                ...
                } else if ('SPAN' == $ele[0].tagName || 'DIV' == $ele[0].tagName || 'LABEL' == $ele[0].tagName) {
                    $ele.text(value);
                } else {
                    $ele.val(value);
    ...
    return {
         ...
         * @param data
         * @param $form
         * @param prefixName 表单控件名称前缀，一般为空
         * @param isReFetchEle  每次遍历时是否重新获取控件
         * @returns {*}
         */
        fill: function (data, $form, prefixName, isReFetchEle) {
            return fill(data, $form, prefixName, isReFetchEle);
        }
    };
};
