...
public class RuleMixKpiHandler {
    ...
    public Future<List<Long>> fetchMixKpi(RuleGroupTask task, RuleKpiDefine kpi){
        List<Long> mixKpi = relationService.queryIdByTargetIdRelType(kpi.getRuleKpiDefineRowId(), new String[]{RuleConstants.Type.MIX_BASE_KPI_REL});
        logger.debug("活动{} KPI {} 账期 {} 完成 查询组合指标  组合指标条数:{}", task.getTaskName(), kpi.getKpiCode(), task.getDateCd(), mixKpi.size());
        if (mixKpi.size() > 0) {
